...
const ChatHeader = ({ roomId, data, isLoading, isError }) => {
  ...
  const menuItems = [
    { label: "👤 View Profile", action: () => navigate(`/user/${otherMember?.user?.id}`) },
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <motion.div
          ...
          onClick={() => otherMember?.user?.id && navigate(`/user/${otherMember.user.id}`)}
        >
          {room.profileImage ? (
            <img
            ...
            <div
              className="w-11 h-11 rounded-full flex items-center justify-center text-xs font-black text-white shadow-md"
          ...
          {isOtherUserOnline && (
            <motion.span
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-500 border-2 border-white dark:border-slate-900 rounded-full shadow-[0_0_8px_rgba(16,185,129,0.6)]"
            />
          )}
        </motion.div>
      ...
      <div className="flex items-center gap-2">
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          className="w-10 h-10 flex items-center justify-center rounded-xl text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-slate-800 transition-all cursor-pointer"
        >
          ...
          <AnimatePresence>
            {isMenuOpen && (
              <motion.div
                ...
                className="absolute right-0 top-full mt-2 w-52 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl shadow-[0_20px_60px_-12px_rgba(0,0,0,0.18)] border border-gray-100 dark:border-slate-800 overflow-hidden z-30 py-2"
              >
                {menuItems.map((item, i) => (
                  <motion.button
                    ...
                    className={`w-full text-left px-4 py-2.5 text-[13px] font-semibold hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors cursor-pointer text-gray-700 dark:text-gray-200 ${item.className || ""}`}
                  >
                    {item.label}
                  </motion.button>
                ))}
