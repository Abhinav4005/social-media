...
const VideoCall = (props) => {
  ...
  // SRP: Media device & stream track management isolated in useMediaStream hook
  const {
    myVideo,
    streamRef,
    hasCamera,
  ...
  // SRP: Peer connection, signaling & call timer state isolated in useWebRTC hook
  const {
    userVideo,
    incomingCall,
    callAccepted,
    ...
    <div className="fixed inset-0 z-40 overflow-hidden" style={{ background: "#0a0a0f" }}>
      ...
      {!callEnded && (
        ...
        >
          ...
          {/* Flip button — swap PIP ↔ main feed */}
          <Btn
            onClick={flipVideos}
      ...
      <div className="absolute inset-0">
        ...
        {/* Main video feed — remote normally, local when swapped */}
        <video
          ref={swapped ? myVideo : userVideo}
          autoPlay
          muted={swapped}   /* mute local when shown as main */
          playsInline
        ...
        {/* Live timer badge — shown on the video when active */}
        {callAccepted && (
          <div
        ...
        {/* PIP tile — shows local normally, remote when swapped */}
        <motion.div
        ...
        >
          ...
          >
            <video
              ...
              autoPlay
              muted={!swapped}   /* mute local feed in PIP */
              playsInline
