    ...
    >
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          >
            {post?.user?.profileImage ? (
              <img src={post.user.profileImage} alt="" className="h-11 w-11 rounded-full object-cover ring-2 ring-white dark:ring-slate-800 shadow-md" />
            ) : (
              <span className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-sm font-semibold text-white shadow-md">
                {initials}
              </span>
            )}
          </motion.div>
          <div className="min-w-0">
            <button
              ...
              className="block truncate text-left text-[14px] font-bold text-gray-950 dark:text-gray-100 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
            >
          ...
          {showMenu && (
            <div className="absolute right-0 mt-2 w-48 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-2xl shadow-xl border border-gray-100 dark:border-slate-800 py-2 z-30 animate-fadeIn text-sm font-medium">
              <button
                ...
                className="w-full px-4 py-2 text-left text-gray-700 dark:text-gray-200 hover:bg-indigo-50 dark:hover:bg-slate-800 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors flex items-center gap-2"
              >
                🔗 Copy Post Link
              </button>
      ...
      {(post?.title || post?.description) && (
        <div className="px-5 py-4">
          ...
          {post?.description && <p className="font-content mt-2 text-[15px] leading-7 text-gray-700 dark:text-gray-300">{post.description}</p>}
        </div>
      )}
      ...
      {(likeCount > 0 || comments.length > 0) && (
        <div className="mx-5 flex items-center justify-between border-b border-gray-100/50 dark:border-slate-800 pb-3 text-xs text-gray-500 dark:text-gray-400">
          ...
          {comments.length > 0 && (
            <button type="button" onClick={() => setOpenCommentModal(true)} className="font-semibold hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
      ...
      <footer className="grid grid-cols-[1fr_1fr_1fr_auto] gap-1 px-4 py-2">
        ...
        <motion.button
          ...
          className={`app-icon-button h-10 w-10 ${saved ? "text-indigo-600 dark:text-indigo-400" : "text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-800"}`}
        >
          <Bookmark className={`h-4 w-4 ${saved ? "fill-indigo-600 dark:fill-indigo-400" : ""}`} />
        </motion.button>
      </footer>
