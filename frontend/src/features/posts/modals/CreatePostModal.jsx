...
export default function CreatePostModal({ isOpen, onClose, isEditing = false, editedPost = {} }) {
  ...
  const handlePost = (e) => {
    ...
    const validation = postSchema.safeParse({
      content: formData.description || "Media attachment",
      visibility: formData.privacy || "PUBLIC",
    });

    if (!validation.success) {
      showError(validation.error.errors[0]?.message || "Invalid post data");
      return;
    }
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-slate-800">
                <div className="flex items-center gap-1">
                  <button
                    ...
                    title="Add Emoji"
                    className={`p-2 rounded-xl text-gray-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 transition-colors cursor-pointer ${showEmojiPicker ? "bg-indigo-50 text-indigo-600 dark:bg-indigo-950/60 dark:text-indigo-400" : ""
                      }`}
                  >
                    <Smile size={18} />
                  </button>
                  ...
                  <button
                    ...
                    title="Add Location"
                    className={`p-2 rounded-xl text-gray-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 transition-colors cursor-pointer ${showLocationInput ? "bg-indigo-50 text-indigo-600 dark:bg-indigo-950/60 dark:text-indigo-400" : ""
                      }`}
                  >
                    <MapPin size={18} />
                  </button>
